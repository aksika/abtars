# Report Pipeline E2E — e2e-1785195413237-fdb5bb16
...
| X01 | Active run blocks stale advancement (extra) | PASS | scheduler |
...
| X02 | Non-report task rejects report contract (extra) | PASS | normalize |
| X03 | Legacy report accepted without contract (extra) | PASS | normalize |
| M08 | Required file missing → preflight fails | PASS | preflight |
| X04 | Required file present → preflight passes | PASS | preflight |
| M09 | Missing executable → preflight fails | PASS | preflight |
| X05 | Preflight rejects path escaping workspace | PASS | preflight |
...
| X06 | requestCancel propagates cancellation (extra) | PASS | runner |
| X07 | markTerminal prevents duplicate (extra) | PASS | runner |
...
| X08 | Failures increment correctly (extra) correctly | PASS | state |
...
| X09 | Rejects artifact mtime before reservation (extra) | PASS | artifact |
...
**57/57 PASS** (0 FAIL, 0 BLOCKED, 0 NOT_RUN)