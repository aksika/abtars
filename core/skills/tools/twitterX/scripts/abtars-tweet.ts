...
// ── Inlined helpers (no abtars deps) ──
import { readFileSync, existsSync, mkdirSync, writeFileSync } from "node:fs";
import { join, basename } from "node:path";
import { homedir } from "node:os";

function abtarsHome(): string { return process.env.ABTARS_HOME ?? join(homedir(), ".abtars"); }
function reportsDir(cat: string): string { return join(abtarsHome(), "reports", cat); }
function localDate(): string { const d = new Date(); return `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,"0")}-${String(d.getDate()).padStart(2,"0")}`; }
function logAndSwallow(_tag: string, _ctx: string, _err?: unknown): undefined { return undefined; }
