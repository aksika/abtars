 ...
 *
 * Full (default): abtars zip + abmind zip (calls `abmind backup`)
...
const ABTARS_EXCLUDE = [
  ...
  "deploy.state", ".bridge.flock", "state/task-queue-state.json", "memory.db",
  "*.sock", "*.db-wal", "*.db-shm",
];
...
export async function backup(opts: BackupOpts = {}): Promise<number> {
  ...
  // 2. Call abmind backup (full), move result to destDir
  if (!isConfig && existsSync(abmindHome)) {
    const abmindFile = runAbmindBackup(abmindHome);
    if (abmindFile) {
      const dest = join(destDir, basename(abmindFile));
      renameSync(abmindFile, dest);
      const size = (statSync(dest).size / 1024 / 1024).toFixed(1);
      process.stdout.write(`✓ ${basename(dest)} (${size}MB)\n`);
    } else {
      process.stderr.write("⚠️ abmind backup failed\n");
    }
  }
