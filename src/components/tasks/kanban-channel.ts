...
export function channelPost(cardId: number, from: string, to: string, message: string, directive = false, msgType = "progress"): number {
  ...
  const stmt = d.prepare("INSERT INTO agent_channel (card_id, from_agent, to_agent, message, directive, msg_type) VALUES (?, ?, ?, ?, ?, ?)");
...
export function channelRead(cardId: number, opts?: { since?: string; from?: string }): ChannelMessage[] {
  ...
  return d.prepare(sql).all(...params) as ChannelMessage[];
}
...
export function channelUnread(cardId: number, agentName: string): ChannelMessage[] {
  ...
  const rows = d.prepare(sql).all(cardId, agentName, `%${agentName}%`) as ChannelMessage[];

  // Mark as seen
  if (rows.length > 0) {
    const markStmt = d.prepare("UPDATE agent_channel SET last_seen_by = last_seen_by || ? WHERE id = ?");
...
export function channelGc(completedCardIds: number[]): number {
  ...
  const result = d.prepare(`DELETE FROM agent_channel WHERE card_id IN (${placeholders})`).run(...completedCardIds);
...
export function channelRetentionGc(activeCardIds: number[]): number {
  ...
  const aged = d.prepare(`DELETE FROM agent_channel WHERE created_at < datetime('now', '-3 days') AND card_id NOT IN (${activeSet})`).run(...activeCardIds);
  // Row cap: keep under 1000
  const count = (d.prepare("SELECT count(*) as c FROM agent_channel").get() as any).c;
  let capped = 0;
  if (count > 1000) {
    const excess = count - 800;
    capped = (d.prepare(`DELETE FROM agent_channel WHERE id IN (SELECT id FROM agent_channel WHERE card_id NOT IN (${activeSet}) ORDER BY created_at ASC LIMIT ?)`).run(...activeCardIds, excess)).changes as number;
  }
...
export function channelPostFromRemote(cardId: number, from: string, message: string, createdAt: string, peer: string): boolean {
  ...
  if (message.length > MAX_MESSAGE_LEN) message = message.slice(0, MAX_MESSAGE_LEN) + "…";
  try {
    const exists = d.prepare("SELECT 1 FROM agent_channel WHERE card_id = ? AND from_agent = ? AND created_at = ?").get(cardId, from, createdAt);
    if (exists) return true; // already have it
    d.prepare("INSERT INTO agent_channel (card_id, from_agent, to_agent, message, created_at, remote_peer, synced) VALUES (?, ?, 'ALL', ?, ?, ?, 1)")
...
export function initChannelSync(): void {
  nerve.on("channel:message", (cardId: number, meta?: { from: string; to: string; message: string }) => {
    ...
    const lastRow = d.prepare("SELECT remote_peer FROM agent_channel WHERE card_id = ? ORDER BY id DESC LIMIT 1").get(cardId) as { remote_peer: string | null } | undefined;
...
async function pushToRemote(peer: string, cardId: number, from: string, message: string): Promise<void> {
  ...
  const d = db();
  if (d) d.prepare("UPDATE agent_channel SET synced = 1 WHERE card_id = ? AND synced = 0").run(cardId);
}
