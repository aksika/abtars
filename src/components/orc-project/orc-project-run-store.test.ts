...
// #1792: the "#1729 v2 synthesis admission" salvage-claim suite lived here —
// every test pinned the deleted `claimSalvageExecution` authority, so the whole
// describe (plus its now-unused `salvageInput`/`reportFile`/`snapshotFor`/seed
// helpers) is deleted with it. No test covered non-salvage synthesis-admission
// behavior, so none is retained. Replacement evidence: explicit writer nodes +
// partial-evidence policy in `orc-workflow-runner.test.ts` and
// `src/tests/integration/scheduled-synthesis.integration.test.ts` — no new tests.
