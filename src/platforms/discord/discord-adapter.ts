...
export class DiscordAdapter implements PlatformAdapter {
  ...
  private _startTypingLoop(channelId: string): void {
    ...
    this.api.sendTyping(channelId).catch(err => logAndSwallow(TAG, "send typing", err));
    const iv = setInterval(() => { this.api.sendTyping(channelId).catch(err => logAndSwallow(TAG, "send typing", err)); }, 8000);
