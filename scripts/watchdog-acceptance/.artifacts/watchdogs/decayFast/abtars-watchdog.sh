...
LOCK="$AB/bridge.lock"
STALE=6        # heartbeat staleness threshold (seconds)
POLL=2          # documented poll cadence
POLL_INTERVAL=0.3  # bounded state-poll slice (R3.5: check durable state <=5s)
...
" 2>/dev/null)
    ...
    (( $(date +%s) - SPAWNED_AT < 2 )) && { sleep "$POLL_INTERVAL"; continue; }
    ...
    _health_now=$(date +%s)
    if (( _health_now - ${LAST_HEALTH_ACCOUNT:-0} >= 1 )); then
